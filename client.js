...
async function sauvegarderEcheanciersSurServeur() {
  try {
    const response = await fetch(ECHANCIERS_URL, {
      ...
      body: JSON.stringify(echeanciers),
    });
    if (!response.ok) throw new Error("Erreur synchro");
    console.log("✅ Échéanciers synchronisés avec le serveur");
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(data)) {
        echeanciers = data;
        console.log(`✅ ${echeanciers.length} échéanciers chargés du serveur`);
        localStorage.setItem(
          "ventespro_echeanciers",
          JSON.stringify(echeanciers),
        );
        return;
      }
    }
    throw new Error("Chargement serveur échoué");
  } catch (error) {
    ...
    if (saved) {
      ...
      console.log(
        `📱 ${echeanciers.length} échéanciers chargés du localStorage`,
      );
    } else {
      echeanciers = [];
    }
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-5xl mx-4 animate-fadeIn max-h-[90vh] overflow-y-auto">
      ...
      <div class="p-6">
        ...
        <div class="flex gap-3 mt-6 pt-4 border-t">
          ...
          <button onclick="forcerSynchronisation()" class="flex-1 bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg transition">
            <i class="fas fa-cloud-upload-alt mr-2"></i> Synchroniser
          </button>
